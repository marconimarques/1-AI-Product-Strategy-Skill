...
This skill works through a structured 7-step framework - Value Stack analysis, data moat design, UX paradigm selection, domain-specific evals, feedback loop architecture, business model alignment, and trust design - and produces a document where every claim is grounded in what you actually told it, every cited figure is labeled with its basis, and every assumption that requires discovery is explicitly called out as a hypothesis rather than dressed up as insight.
...
The skill works through all 7 steps applied to your specific product. It does not produce generic examples. Each step reasons from what you described.
...
| 2. Data Moat | Input data, feedback signals, and the proprietary context layer that makes the product's outputs hard to replicate |
| 3. AI UX Paradigm | Choice between Assistant, Agent, Autonomous, or Embedded Intelligence - with rationale and an evolution path |
| 4. Domain-Specific Evals | What "good" means in your domain by business outcome, not model quality metrics |
| 5. Feedback Loop Architecture | Micro, meso, and macro loops designed from the start to compound AI advantage over time |
| 6. Business Model Alignment | Inference cost estimate, model-mixing plan, pricing structure, and a unit economics check at 10x scale |
| 7. Trust Architecture | Transparency mechanisms, user control mechanisms, and a progressive autonomy path |
